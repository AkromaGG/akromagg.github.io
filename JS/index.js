...
function fetchProjectData(projectName) {
  ...
  return fetch(projectJsonPath)
    .then((response) => {
      if (!response.ok)
        throw new Error(
          `Failed to load project.json for ${projectName}: ${response.status}`
        );
    ...
    .then((projectData) => {
      ...
      return {
        ...
        alt: title,
        galleryPageUrl: `../Projects/${projectName}/${
...
if (!thumbnailContainer) {
  ...
  fetchProjects().then((projects) => {
      ...
      fetchProjectData(projectName).then((artwork) => {
        ...
        if (!bannerImageSet && artwork.bannerImageUrl) {
          document.querySelector(
            ".top-container"
          ).style.backgroundImage = `url(${artwork.bannerImageUrl})`;
          bannerImageSet = true;
        }
      })
    );
    ...
    Promise.all(projectPromises).then(() => {
      ...
      if (announcer)
        announcer.textContent = `Loaded ${projects.length} projects`;
    });
  });
}
